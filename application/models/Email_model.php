...
class Email_model extends CI_Model
{
    ...
    public function sendEmail($data)
    {
        ...
        // Parse placeholders for BOTH subject and message
        $extra = [];
        if (!empty($data['branch_id'])) {
            $branchName = get_type_name_by_id('branch', $data['branch_id']);
            if (!empty($branchName)) {
                $extra['{institute_name}'] = $branchName;
                $extra['{branch_name}']    = $branchName; // optional alias
            }
        }
        $data['subject'] = $this->replace_tags($data['subject'], $extra);
        $data['message'] = $this->replace_tags($data['message'], $extra);

        return $this->mailer->send($data) ? true : false;
    }
    ...
    private function replace_tags($text, array $extra = [])
    {
        $tags = array_merge([
            '{institute_name}' => get_global_setting('institute_name'),
            '{institute_code}' => get_global_setting('institution_code'),
            '{base_url}'       => base_url(),
        ], $extra);
