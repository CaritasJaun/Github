...
public function get_pending_scores($student_ids = [])
{
    if (empty($student_ids)) return [];

    return $this->db->select('sap.*, s.first_name, s.last_name, sub.name as subject_name')
        ->from('student_assign_paces as sap')
        ->join('student as s', 's.id = sap.student_id')
        ->join('subject as sub', 'sub.id = sap.subject_id', 'left')
        ->where_in('sap.student_id', $student_ids)
        ->where('sap.status', 'assigned') // still assigned
        ->where('sap.first_attempt_score IS NULL') // no 1st score
        ->where('sap.second_attempt_score IS NULL') // no 2nd score either
        ->get()->result_array();
}
}
