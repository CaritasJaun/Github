		...
		<div class="profile-head social">
			...
			<div class="col-md-12 col-lg-5 col-xl-5">
				...
				<ul>
					...
					<li><div class="icon-holder" data-toggle="tooltip" data-original-title="<?=translate('home_address')?>"><i class="fas fa-home"></i></div> <?=(!empty($staff['present_address']) ? $staff['present_address'] : 'N/A'); ?></li>
				</ul>
			</div>
		</div>
	</div>
...
<?php echo form_open_multipart('employee/profile/' . (int)$staff['id'], ['id' => 'profileForm', 'method' => 'post']); ?>
	<input type="hidden" name="staff_id" id="staff_id" value="<?php echo (int)$staff['id']; ?>">
	<?php if (is_superadmin_loggedin()): ?>
		<input type="hidden" name="branch_id" value="<?php echo (int)$staff['branch_id']; ?>">
	<?php endif; ?>
					...
					<div class="form-group">
						...
						<?php
							...
							echo form_dropdown(
								"branch_id",
								$arrayBranch,
								set_value('branch_id', $staff['branch_id']),
								"class='form-control' id='branch_id' data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity'"
							);
						?>
			...
			<?php } ?>
				...
				<div class="col-md-<?=$widget?> mb-sm">
					<div class="form-group">
						...
						<?php
							// Allow Principal (6). Exclude only Super Admin (1), Student (7), Parent (9).
							$role_list = $this->app_lib->getRoles([1, 7, 9]);
							echo form_dropdown(
								'user_role',
								$role_list,
								set_value('user_role', $staff['role_id']),
					...
					<div class="form-group">
						...
						<?php
							...
							echo form_dropdown(
								"designation_id",
								$designation_list,
								set_value('designation_id', $staff['designation']),
								"class='form-control' id='designation' data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity'"
							);
						?>
					...
					<div class="form-group">
						...
						<?php
							...
							echo form_dropdown(
								"department_id",
								$department_list,
								set_value('department_id', $staff['department']),
								"class='form-control' id='department_id' data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity'"
							);
						?>
				...
				<div class="col-md-4 mb-sm">
					<div class="form-group">
						...
						<?php
							...
							echo form_dropdown("sex", $array, set_value('sex', $staff['sex']), "class='form-control' data-plugin-selectTwo data-width='100%' data-minimum-results-for-search='Infinity'");
						?>
					</div>
				</div>
			</div>
	...
	<div class="col-md-12 mb-sm">
		<div class="form-group">
			<label class="control-label"><?=translate('home_address')?> <span class="required">*</span></label>
			<textarea class="form-control" rows="2" name="home_address" placeholder="<?=translate('home_address')?>"><?=set_value('home_address', $staff['present_address'])?></textarea>
			<span class="error"><?php echo form_error('home_address'); ?></span>
		</div>
	</div>
</div>
		...
		<div class="panel-footer">
			<div class="row">
				<div class="col-md-offset-9 col-md-3">
					<button id="updateBtn" type="submit" class="btn btn-default btn-block" form="profileForm">
						<?= translate('update') ?>
					</button>
				</div>
			</div>
...
<script>
// Fallback: force submit even if some script called preventDefault()
(function () {
  ...
  form.addEventListener('submit', function () { form._submitted = true; });
})();
</script>
		...
		<?php echo form_open_multipart('employee/document_create', array('class' => 'form-horizontal frm-submit-data')); ?>
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<?php
							echo form_dropdown("document_category", $categorylist, set_value('document_category'), "class='form-control' data-plugin-selectTwo data-width='100%' id='adocument_category' data-minimum-results-for-search='Infinity'");
						?>
						<span class="error"></span>
					</div>
				</div>
		...
		<?php echo form_open_multipart('employee/document_update', array('class' => 'form-horizontal frm-submit-data')); ?>
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-md-9">
						<?php
							echo form_dropdown("document_category", $categorylist, set_value('document_category'), "class='form-control' data-plugin-selectTwo id='edocument_category' data-width='100%' data-minimum-results-for-search='Infinity'");
						?>
						<span class="error"></span>
					</div>
				</div>
...
(function () {
  ...
  for (var i = 0; i < scripts.length; i++) {
    ...
    if (/\bif\s*\(\s*toast\s*:/.test(t)) {
      var m = t.match(/title\s*:\s*(['"])(.*?)\1/);
      var msg = m ? m[2] : 'Error';
