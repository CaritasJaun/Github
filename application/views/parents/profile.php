	...
	<div class="col-md-12 mb-lg">
		<div class="profile-head social">
			...
			<div class="col-md-12 col-lg-5 col-xl-5">
				...
				<ul>
					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="<?=translate('relation')?>">
							<i class="fas fa-bezier-curve"></i>
						</div>
						<?=html_escape($parent['relation'])?>
					</li>

					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="Father Mobile No">
							<i class="fas fa-phone"></i>
						</div>
						<?=html_escape(empty($parent['father_mobileno']) ? 'N/A' : $parent['father_mobileno']);?>
					</li>

					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="Mother Mobile No">
							<i class="fas fa-phone"></i>
						</div>
						<?=html_escape(empty($parent['mother_mobileno']) ? 'N/A' : $parent['mother_mobileno']);?>
					</li>

					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="Father Email">
							<i class="far fa-envelope"></i>
						</div>
						<?=html_escape(!empty($parent['father_email']) ? $parent['father_email'] : 'N/A')?>
					</li>

					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="Mother Email">
							<i class="far fa-envelope"></i>
						</div>
						<?=html_escape(!empty($parent['mother_email']) ? $parent['mother_email'] : 'N/A')?>
					</li>

					<li>
						<div class="icon-holder" data-toggle="tooltip" data-original-title="<?=translate('address')?>">
							<i class="fas fa-home"></i>
						</div>
						<?=html_escape(!empty($parent['address']) ? $parent['address'] : 'N/A'); ?>
					</li>
				</ul>
			</div>
		</div>
			...
			<div class="panel panel-accordion">
				...
				<div id="profile" class="accordion-body collapse <?=($this->session->flashdata('profile_tab') ? 'in' : ''); ?>">
					...
					<div class="panel-body">
						...
						<!-- NEW: Father/Mother Mobile -->
						<div class="row">
							<div class="col-md-6 mb-sm">
								<div class="form-group">
									<label class="control-label">Father Mobile No</label>
									<input class="form-control" name="father_mobileno" type="text"
										value="<?=set_value('father_mobileno', $parent['father_mobileno'])?>" autocomplete="off" />
								</div>
							</div>
							<div class="col-md-6 mb-sm">
								<div class="form-group">
									<label class="control-label">Mother Mobile No</label>
									<input class="form-control" name="mother_mobileno" type="text"
										value="<?=set_value('mother_mobileno', $parent['mother_mobileno'])?>" autocomplete="off" />
								</div>
							</div>
						</div>

						<!-- NEW: Father/Mother Email -->
						<div class="row">
							<div class="col-md-6 mb-sm">
								<div class="form-group">
									<label class="control-label">Father Email</label>
									<input type="email" class="form-control" name="father_email"
										value="<?=set_value('father_email', $parent['father_email'])?>" autocomplete="off" />
								</div>
							</div>
							<div class="col-md-6 mb-sm">
								<div class="form-group">
									...
									<input type="email" class="form-control" name="mother_email"
										value="<?=set_value('mother_email', $parent['mother_email'])?>" autocomplete="off" />
								</div>
							</div>
						</div>
			...
			<div class="panel panel-accordion">
				...
				<div id="childs" class="accordion-body collapse">
					<div class="panel-body">
						...
						<div class="row">
							<div class="col-md-12">
								<form action="<?= base_url('parents/attach_children/' . (int)$parent['id']); ?>" method="post" class="mb-md">
									...
									<div class="form-group">
										<label class="control-label"><?= translate('add_children'); ?></label>
										<select name="student_ids[]" class="form-control" data-plugin-selectTwo data-width="100%" multiple>
											<?php if (!empty($students_for_select)): ?>
												<?php foreach ($students_for_select as $s): ?>
													<option value="<?= (int)$s['id']; ?>">
														<?= html_escape($s['full_name']); ?>
														<?php if (!empty($s['register_no'])): ?>
															(<?= html_escape($s['register_no']); ?>)
														<?php endif; ?>
													</option>
												<?php endforeach; ?>
											<?php endif; ?>
										</select>
										<span class="help-block"><?= translate('hold_ctrl_to_select_multiple'); ?></span>
									</div>

									<button type="submit" class="btn btn-primary">
										<?= translate('attach_selected'); ?>
									</button>
								</form>
							</div>
						</div>
