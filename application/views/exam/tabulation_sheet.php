							...
							<tbody>
								...
								if (count($students_list)) {
									foreach ($students_list as $enroll) {
										// --- set up per-student array (unchanged) -----------------
										$studentArray                             = array();
								...
								if (!empty($studentArrayList)):
									foreach ($studentArrayList as $row1): ?>
										<tr>
												...
												<td>
													...
													if (!isset($subject['mark_empty']) || $subject['mark_empty'] !== true) {
														if (!isset($subject['absent']) || $subject['absent'] !== true) {
															if ($subject['totalFullMark'] > 0) {
																echo number_format(($subject['totalObtained'] / $subject['totalFullMark']) * 100, 1, '.', '') . ' %';
															} else {
																echo '0 %';
															}
														} else {
											...
											<td>
												<?php
												$subjectPercentages = 0;
												$totalSubjects      = count($row1['subject_array_list']);
												foreach ($row1['subject_array_list'] as $subj) {
													if (isset($subj['totalFullMark']) && $subj['totalFullMark'] > 0) {
														$subjectPercentages += ($subj['totalObtained'] / $subj['totalFullMark']) * 100;
													}
												}
												$averagePercentage = ($totalSubjects > 0) ? ($subjectPercentages / $totalSubjects) : 0;
												echo number_format($averagePercentage, 1, '.', '') . ' %';
												?>
											</td>
										</tr>
									<?php
