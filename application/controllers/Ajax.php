...
public function get_events_list()
{
    ...
    // This install uses event.start_date / end_date and event.type -> event_types.id
    $start_col = $this->db->field_exists('start_date','event') ? 'start_date' : 'start';
    $end_col   = $this->db->field_exists('end_date','event')   ? 'end_date'   : 'end';

    $this->db->from('event e');
    // Join the type table used elsewhere in your views: event_types
    $this->db->join('event_types et', 'et.id = e.type', 'left');

    // Prefer explicit color on event_types.color; fall back to e.color if present; give holiday a default
    $colorExpr = "COALESCE(et.color, " . ($this->db->field_exists('color','event') ? "e.color, " : "") . 
                 "(CASE WHEN e.type='holiday' THEN '#F7DC6F' END))";

    $this->db->select("e.id, e.title, e.{$start_col} AS start, e.{$end_col} AS end, {$colorExpr} AS color", false);

    if ($this->db->field_exists('branch_id','event') && !empty($branch_id)) {
        $this->db->where('e.branch_id', $branch_id);
    }
    if ($this->db->field_exists('session_id','event')) {
        $this->db->where('e.session_id', $session_id);
    }

    if (!empty($start) && !empty($end)) {
        $this->db->where("e.{$start_col} <=", $end);
        $this->db->where("e.{$end_col} >=", $start);
    }
    ...
    foreach ($rows as $r) {
        ...
        $events[] = [
            ...
            'end'             => (string)$r['end'],
            'allDay'          => (strlen($r['start']) === 10 && strlen($r['end']) === 10),
            // send all common keys so every FullCalendar config uses the same color
            'color'           => $c,
            'backgroundColor' => $c,
            'borderColor'     => $c,
        ];
