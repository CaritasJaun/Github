    ...
    public function index()
    {
        ...
        // ---------- STUDENT ----------
            ...
            if ($_POST) {
                ...
                if (isset($validArr['admission_date'])) $this->form_validation->set_rules('admission_date', translate('admission_date'), 'trim|required');
                ...
                if (isset($validArr['first_name']))        $this->form_validation->set_rules('first_name', translate('first_name'), 'trim|required');
                if (isset($validArr['last_name']))         $this->form_validation->set_rules('last_name', translate('last_name'), 'trim|required');
                if (isset($validArr['gender']))            $this->form_validation->set_rules('gender', translate('gender'), 'trim|required');
                if (isset($validArr['birthday']))          $this->form_validation->set_rules('birthday', translate('birthday'), 'trim|required');
                if (isset($validArr['category']))          $this->form_validation->set_rules('category_id', translate('category'), 'trim|required');
                if (isset($validArr['religion']))          $this->form_validation->set_rules('religion', translate('religion'), 'trim|required');
                if (isset($validArr['caste']))             $this->form_validation->set_rules('caste', translate('caste'), 'trim|required');
                if (isset($validArr['blood_group']))       $this->form_validation->set_rules('blood_group', translate('blood_group'), 'trim|required');
                if (isset($validArr['mother_tongue']))     $this->form_validation->set_rules('mother_tongue', translate('mother_tongue'), 'trim|required');
                if (isset($validArr['present_address']))   $this->form_validation->set_rules('current_address', translate('present_address'), 'trim|required');
                if (isset($validArr['permanent_address'])) $this->form_validation->set_rules('permanent_address', translate('permanent_address'), 'trim|required');
                if (isset($validArr['city']))              $this->form_validation->set_rules('city', translate('city'), 'trim|required');
                if (isset($validArr['state']))             $this->form_validation->set_rules('state', translate('state'), 'trim|required');
                if (isset($validArr['student_email']))     $this->form_validation->set_rules('email', translate('email'), 'trim|required|valid_email');
                if (isset($validArr['student_mobile_no'])) $this->form_validation->set_rules('mobileno', translate('mobile_no'), 'trim|required|numeric');
        ...
        // ---------- STAFF (Admin, Principal, Teacher, etc.) ----------
        } else {
            if ($_POST) {
                $this->form_validation->set_rules('name', translate('name'), 'trim|required');
            ...
            // --- Fallback: if Principal mapping is wrong (login_credential.user_id), try resolve by username/email.
            if (empty($this->data['staff'])) {
                $lc = $this->db->select('id, user_id, role, username, email')
                               ->where('id', get_loggedin_id())
                               ->get('login_credential')->row();
                if ($lc && (int)$lc->role === 6) { // 6 = Principal
                    $byEmail = $this->db->where('email', $lc->username)
                                        ->or_where('email', $lc->email)
                                        ->get('staff')->row_array();
                    if (!empty($byEmail)) {
                        $this->data['staff'] = $byEmail;
                        // Optional self-heal of mapping:
                        // if ((int)$lc->user_id !== (int)$byEmail['id']) {
                        //     $this->db->where('id', $lc->id)->update('login_credential', ['user_id' => (int)$byEmail['id']]);
                        // }
                    }
                }
            }
        ...
        // Prevent layout permission gate from 404'ing if role lacks explicit 'profile' permission
        if (function_exists('get_permission') && !get_permission('profile', 'is_view')) {
