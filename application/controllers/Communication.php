   ...
   public function getStafflistRole()
{
    ...
    if (!empty($branch_id)) {
        $role_id     = (int) $this->input->post('role_id');          // e.g. 1,2,3,4,6,8
        $selected_id = (int) ($this->input->post('staff_id') ?? 0);

        $this->db->select('staff.id, staff.name, staff.staff_id');
        $this->db->from('staff');
        // ✅ only join staff roles to avoid collisions with parent(5)/student(7)
        $this->db->join(
            'login_credential as lc',
            'lc.user_id = staff.id AND lc.role IN (1,2,3,4,6,8)',
            'inner'
        );
        $this->db->where('lc.role', $role_id);     // includes Principal=6 correctly
        ...
        if (count($result)) {
            ...
            foreach ($result as $staff) {
                if ((int)$staff['id'] === (int)get_loggedin_user_id()) continue;
                $sel = ($staff['id'] == $selected_id ? 'selected' : '');
                $html .= "<option value='{$staff['id']}' {$sel}>{$staff['name']} ({$staff['staff_id']})</option>";
            }
        } else {
    ...
    public function getParentListBranch()
{
    ...
    $branch_id = $this->application_model->get_branch_id();
    if (!empty($branch_id)) {
        $selected_id = (int) ($this->input->post('parent_id') ?? 0);

        $this->db->select('p.id, p.name');
        $this->db->from('parent as p');
        $this->db->join('login_credential as lc', 'lc.user_id = p.id AND lc.role = 5 AND lc.active = 1', 'inner');
        ...
        if (count($result)) {
            $html .= "<option value=''>" . translate('select') . "</option>";
            foreach ($result as $row) {
                if ((int)$row['id'] === (int)get_loggedin_user_id()) continue;
