...
{
    $role = get_loggedin_role_id(); // 1=Admin, 2=Teacher, 3=Parent

    // ❌ Block Parents entirely
    if ($role == 3) {
        access_denied();
    }

    // ✅ Teachers must have permission to view
    if ($role == 2 && !get_permission('generate_student_certificate', 'is_view')) {
        access_denied();
    }

    // ✅ Admins must have permission too (optional safeguard)
    if ($role == 1 && !get_permission('generate_student_certificate', 'is_view')) {
        access_denied();
    }

    // ✅ Proceed with loading student list and page data
