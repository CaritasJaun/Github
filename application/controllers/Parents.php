...
class Parents extends Admin_Controller
{
    ...
    protected function parent_validation()
    {
        ...
        $this->form_validation->set_rules('father_email', 'Father Email', 'trim|valid_email');
        $this->form_validation->set_rules('mother_email', 'Mother Email', 'trim|valid_email');
        $this->form_validation->set_rules('father_mobileno', 'Father Mobile No', 'trim');
        $this->form_validation->set_rules('mother_mobileno', 'Mother Mobile No', 'trim');
        ...
        // --- CHANGED: username is OPTIONAL; if present must be a valid email and unique.
        // This mirrors the relaxed rules we applied for Students/Employees so updates
        // don't fail when many legacy usernames are NULL.
        if ($getBranch['grd_generate'] == 0 || isset($_POST['parent_id'])) {
            $this->form_validation->set_rules('username', translate('username'), 'trim|valid_email|callback_unique_username');
...
public function view()
{
    ...
    $this->data['parentslist'] = $this->parents_model->getParentList($this->data['branch_id'], 1); // <-- add this
    ...
    public function profile($id = '')
    {
        ...
        $this->data['students_for_select'] = $this->parents_model->getStudentsForAttach($this->data['parent']['branch_id'], (int)$id);
    ...
    public function attach_children($id = '')
{
    if (!get_permission('parent', 'is_edit')) {
        access_denied();
    }
    $parent_id  = (int)$id;
    $student_ids = (array)($this->input->post('student_ids') ?? []);

    if ($parent_id <= 0) {
        set_alert('error', 'Invalid parent.');
        redirect(base_url('parents/view'));
    }
    if (empty($student_ids)) {
        set_alert('error', translate('please_select_at_least_one_item'));
        redirect(base_url('parents/profile/' . $parent_id));
    }

    $ok = $this->parents_model->attachChildren($parent_id, $student_ids);

    if ($ok) {
        // By sharing the same parent_id they are siblings across the system.
        set_alert('success', 'Selected students have been linked to this parent (siblings created).');
    } else {
        set_alert('error', 'Could not link students. Please try again.');
    }
    redirect(base_url('parents/profile/' . $parent_id));
}
