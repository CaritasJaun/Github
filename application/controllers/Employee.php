...
{
    // old links like /employee/password → send to the real page
    redirect(base_url('profile/password'));
}

public function change_password()
{
    // catch any variant the UI might use
    redirect(base_url('profile/password'));
}
    ...
    protected function employee_validation()
    {
        ...
        // CHANGED: use single Home Address instead of Present Address
        $this->form_validation->set_rules('home_address', translate('home_address'), 'trim|required');
...
{
    // block Super Admin(1) and Student(7) tabs
    if (!get_permission('employee', 'is_view') || in_array((int)$role, [1, 7], true)) {
        return access_denied();
    }

    // Active tab / role id (fallback to URL segment or Admin=2)
    $role_id = (int) ($role ?: $this->uri->segment(3) ?: 2);
    $this->data['act_role'] = $role_id;

    // Branch scope:
    // - Non-SA: force to their branch
    // - SA: show all branches unless ?branch_id=<id> is provided
    if (is_superadmin_loggedin()) {
        $branch_id = (int) $this->input->get('branch_id');
        if ($branch_id <= 0) {
            $branch_id = null; // no branch filter for SA by default
        }
    } else {
        $branch_id = (int) get_loggedin_branch_id();
    }

    // Pull strictly by LOGIN ROLE (source of truth)
    $this->data['stafflist'] = $this->employee_model->get_list_by_role($role_id, $branch_id);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $customField = $this->input->post("custom_fields[$class_slug]");
                    ...
                    saveCustomFields($customField, $empID, $class_slug);
                }
    ...
    public function profile($id = '')
    {
        ...
        if ($this->input->post('submit') == 'update') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                // Some roles (e.g., Teacher, Principal) may not have employee->is_edit permission.
                // Redirect them somewhere they can always access.
                $redirectTo = get_permission('employee', 'is_edit')
                    ? 'employee/profile/' . $id
                    : 'dashboard';
    ...
    public function delete($id = '')
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where_not_in('role', array(1, 7)); // <-- allow principals
    ...
    /* allow Principal role in the employee form */
    public function valid_role($id)
    {
        $id = (int)$id;
        // Disallow only Super Admin (1), Student (7), Parent (9)
        if (in_array($id, [1, 7, 9], true)) {
    ...
    // --- Added: validation callback for profile picture upload ---
    public function photoHandleUpload($str, $field)
    {
        // If no file chosen, allow validation to pass.
        if (empty($_FILES[$field]['name'])) {
            return true;
        }
        $this->load->library('upload');
        $uploadPath = FCPATH . 'uploads/images/staff/';
        ...
        // Make the uploaded filename available to the controller
